...
/*private LoaderCallbacks<Cursor> changevalues=new LoaderCallbacks<Cursor>() {
    @Override
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        ...
        return new CursorLoader(getApplicationContext(),contract.loginEntry.CONTENT_URI,projection,null,null,null);
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if(data!=null)
        { int username=data.getColumnIndex(contract.loginEntry.c1username);
int name=data.getColumnIndex(contract.loginEntry.c8name);
while(data.moveToFirst())
    {
        if(data.getString(username).equals(MainActivity.USERNAME)&&data.getString(name).equals(venue_name))
        {int rating=data.getColumnIndex(contract.loginEntry.c3rating);
        int from=data.getColumnIndex(contract.loginEntry.c5from_time);
        int to=data.getColumnIndex(contract.loginEntry.c6to_time);
        int visit=data.getColumnIndex(contract.loginEntry.c4visited);
       int info=data.getColumnIndex(contract.loginEntry.c7info);
       data.set
        }
    }
}
